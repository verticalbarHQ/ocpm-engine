...
def test_execute_and_version_check_use_cursor_protocol() -> None:
    engine = OcpmEngine(9, 12)
    version_cursor = FakeCursor(("0.4.0",))
    query_cursor = FakeCursor(({"nodes": []},))

    assert engine.verify_pg_ocpm(version_cursor) == "0.4.0"
...
def test_version_check_rejects_older_pg_ocpm() -> None:
    with pytest.raises(RuntimeError, match="0.4.0 or later"):
        OcpmEngine.verify_pg_ocpm(FakeCursor(("0.3.0",)))
